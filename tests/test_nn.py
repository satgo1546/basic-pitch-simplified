...
class TestHarmonicStacking(unittest.TestCase):
    ...
    def test_defaults(self) -> None:
        ...
        x = nmp.get_cqt(inputs, 5, True)
        conv = nmp.HarmonicStacking(3, [1, 2, 3, 4, 5], N_FREQ_BINS_CONTOURS)(x)
    ...
    def test_fractions(self) -> None:
        inputs = tf.keras.Input(shape=(AUDIO_N_SAMPLES, 1))
        x = nmp.get_cqt(inputs, 2, True)
        conv = nmp.HarmonicStacking(3, [0.5, 1, 2], N_FREQ_BINS_CONTOURS)(x)
        outputs = tfkl.Conv2D(1, (3, 3), padding="same")(conv)
        model = tf.keras.Model(inputs=inputs, outputs=outputs)
